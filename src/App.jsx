...
const CHORD_RE =
  /^[A-G][b#]?(?:maj7|maj|min|m|dim|aug|sus[24]?|add\d|M)?(?:[679]|11|13)?(?:sus[24]?)?(?:\([b#]?\d+\))*(?:\/[A-G][b#]?)?$/;
    ...
    <div key={key} style={{ marginBottom: 14, overflowX: "auto" }}>
      ...
      >
        ...
        {segs.map((seg, i) => (
          ...
          >
            <span style={{ lineHeight: 1.6, marginBottom: 2 }}>
              <ChordTag chord={seg.chord} />
            </span>
            <span
              style={{
                color: "#ddd",
...
export default function App() {
  ...
  const save = () => {
    ...
    if (editing !== null) {
      setSongs((prev) =>
        prev.map((x, i) => (i === editing ? { ...x, ...entry } : x)),
      );
    } else {
    ...
    >
          ...
          {songs.map((s, i) => (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => i < songs.length - 1 && swap(i, i + 1)}
                  style={{
                    background: "#1e1e1e",
                    border: "none",
                    color: "#777",
      ...
      {mode === "perform" && songs.length > 0 && (
        <div style={{ padding: "14px", maxWidth: 700, margin: "0 auto" }}>
          ...
          >
            <button
              onClick={() => goTo(Math.max(0, cur - 1))}
              disabled={cur === 0}
              style={{
                background: cur === 0 ? "#0a0a0a" : "#141414",
                color: cur === 0 ? "#2a2a2a" : "#aaa",
                border: "1px solid #1e1e1e",
                borderRadius: 12,
                padding: "18px 16px",
                fontSize: 15,
            ...
            <button
              onClick={() => goTo(Math.min(songs.length - 1, cur + 1))}
              disabled={cur === songs.length - 1}
              style={{
                background: cur === songs.length - 1 ? "#0a0a0a" : "#141414",
                color: cur === songs.length - 1 ? "#2a2a2a" : "#aaa",
                border: "1px solid #1e1e1e",
                borderRadius: 12,
                padding: "18px 16px",
                fontSize: 15,
