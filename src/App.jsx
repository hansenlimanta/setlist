      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", gap: "0px" }}>
        {prefix ? (
          <span style={{ display: "inline-flex", flexDirection: "column", verticalAlign: "bottom" }}>
            ...
            <span style={{ color: "#ddd", fontSize: 14, fontFamily: "monospace", lineHeight: 1.6, whiteSpace: "pre" }}>{prefix}</span>
          </span>
        ) : null}
        {segs.map((seg, i) => (
          <span key={i} style={{ display: "inline-flex", flexDirection: "column", verticalAlign: "bottom" }}>
            ...
            <span style={{ color: "#ddd", fontSize: 14, fontFamily: "monospace", lineHeight: 1.6, whiteSpace: "pre" }}>
              {seg.lyric || " "}
            </span>
          </span>
        ))}
...
function renderSheet(text) {
  ...
  while (i < lines.length) {
    ...
    out.push(
      <div key={i} style={{ color: "#ccc", fontSize: 14, fontFamily: "monospace", lineHeight: 1.7, marginBottom: 2, whiteSpace: "pre-wrap" }}>{line}</div>
    );
    i++;
  }
  return out;
    ...
    <div style={{ minHeight: "100vh", background: "#0c0c0c", color: "#f0ece0", fontFamily: "monospace" }}>
      ...
      {mode === "perform" && songs.length > 0 && (
        <div style={{ padding: "14px", maxWidth: 700, margin: "0 auto" }}>
          ...
          {/* Chord sheet */}
          <div style={{ background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: 14, padding: "18px 16px", marginBottom: 12 }}>
